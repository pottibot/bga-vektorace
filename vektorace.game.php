    ...
    */
    ...
    function declareGear($n) {
        if ($this->checkAction('declareGear')) {
            ...
            if ($gearProp == 'tireCost' || $gearProp == 'nitroCost')  {
                ...
                $log = clienttranslate('${player_name} spends ${cost} ${tire_token}${nitro_token} to performs a ${shiftType}');
                $log = str_replace((($type == 'tire')? '${nitro_token}' : '${tire_token}'),'',$log);

                self::notifyAllPlayers('gearShift', $log, array(
                    'i18n' => array('shiftType'/* ,'tokenName' */),
                    'player_name' => self::getActivePlayerName(),
                    'player_id' => self::getActivePlayerId(),
                    'shiftType' => (($type == 'tire')? clienttranslate('a downshift') : clienttranslate('an upshift')),
                    'step' => $cost + 1,
                    'cost' => $cost,
                    //'tokenName' => (($type == 'tire')? clienttranslate('tire token') : clienttranslate('nitro token')),
                    'tokenType' => $type,
    ...
    function engageManeuver($enemy, $action, $posIndex) {
        if ($this->checkAction('engageManeuver')) {
            ...
            $playerCurve = self::getCurveObject(self::getPlayerCurve($id)['number']);
    ...
    function stEmergencyBrake() {
        ...
        while ($shiftedGear > 0) {
            ...
            if ($gearPlacement['hasValid']) {
                ...
                // NOTIFY PLAYERS
                self::notifyAllPlayers('useNewVector', clienttranslate('${player_name} spends ${tireExpense} ${tire_token} to downshifts to the ${gearNum} gear'), array(
                    'player_name' => self::getActivePlayerName(),
                    'player_id' => $id,
        ...
        if ($nextCurveNumber != 1) {
            ...
            if (VektoracePoint::dot($pwVec,$carVec) > 0) {
                ...
                if ($playerLapNum == self::getGameStateValue("number_of_laps")) {
                    ...
                    if (self::getGameStateValue('racing_players_number') == 1) $this->gamestate->nextState('raceEnd');
    ...
    function stNextPlayer() {
        ...
        if (self::getPlayerTurnPos($np_id) == 1) {
            ...
            self::notifyAllPlayers('nextRoundTurnOrder', clienttranslate('A new game round begins'), array(
                'order' => $order['list'],
                'missingPlayers' => self::getGameStateValue('racing_players_number')
            ));
